...
async def fetch_data(session, page):
    async with session.get(f'https://esi.evetech.net/dev/universe/types/?datasource=tranquility&page={page}') as response:
...
async def main():
    ...
    for item in all_data:
        async with aiohttp.ClientSession() as session:
            async with session.get(f'https://esi.evetech.net/dev/universe/types/{item}/?datasource=tranquility&language=en') as response:
